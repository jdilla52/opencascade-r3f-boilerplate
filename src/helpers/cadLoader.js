...
const loadSTEPorIGES = async (fileName, inputFile)=> {
    ...
    // this is gross -
    const virtualName = 'test.stp'
    openCascade.FS.createDataFile("/", virtualName, inputFile, true, true, true);
    ...
    const readResult = reader.ReadFile(virtualName);            // Read the file
    if (readResult === openCascade.IFSelect_ReturnStatus.IFSelect_RetDone) {
        ...
        // we'll get this out of here if we want to interface with the breps or anything in the future.
        const mesh = await cascadeToThree(stepShape);
        console.log(fileName + " triangulated and added to the scene!");

        // Remove the file when we're done (otherwise we run into errors on reupload)
        openCascade.FS.unlink(virtualName);
        return mesh
    } else {
